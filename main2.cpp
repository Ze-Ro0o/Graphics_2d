...
//used for filling quarters with circles
...
//used for filling quarters with lines
void DrawLineParametric(HDC hdc, int x1, int y1, int x2, int y2, COLORREF color);//function declaration
void Draw8PointsFillCircleWithLine(HDC hdc, int xc, int yc, int a, int b, COLORREF color, int Quarter)
{
    if (Quarter == 1)
    {
        ...
        SetPixel(hdc, xc - a, yc - b, color);
        SetPixel(hdc, xc + a, yc - b, color);
        DrawLineParametric(hdc, xc, yc, xc + a, yc - b, color);
        SetPixel(hdc, xc + b, yc + a, color);
        SetPixel(hdc, xc - b, yc + a, color);
        SetPixel(hdc, xc - b, yc - a, color);
        SetPixel(hdc, xc + b, yc - a, color);
        DrawLineParametric(hdc, xc, yc, xc + b, yc - a, color);
    }
    else if (Quarter == 2)
    {
        SetPixel(hdc, xc + a, yc + b, color);
    ...
    else if (Quarter == 3)
    {
        SetPixel(hdc, xc + a, yc + b, color);
        SetPixel(hdc, xc - a, yc + b, color);
        DrawLineParametric(hdc, xc, yc, xc - a, yc + b, color); // 3
        SetPixel(hdc, xc - a, yc - b, color);
        SetPixel(hdc, xc + a, yc - b, color);
        SetPixel(hdc, xc + b, yc + a, color);
        SetPixel(hdc, xc - b, yc + a, color);
        DrawLineParametric(hdc, xc, yc, xc - b, yc + a, color); // 3
        SetPixel(hdc, xc - b, yc - a, color);
        SetPixel(hdc, xc + b, yc - a, color);
    }
    else if (Quarter == 4)
    {
        SetPixel(hdc, xc + a, yc + b, color);
        DrawLineParametric(hdc, xc, yc, xc + a, yc + b, color); // 4
        SetPixel(hdc, xc - a, yc + b, color);
        SetPixel(hdc, xc - a, yc - b, color);
        SetPixel(hdc, xc + a, yc - b, color);
        SetPixel(hdc, xc + b, yc + a, color);
        DrawLineParametric(hdc, xc, yc, xc + b, yc + a, color); // 4
        SetPixel(hdc, xc - b, yc + a, color);
        SetPixel(hdc, xc - b, yc - a, color);
        SetPixel(hdc, xc + b, yc - a, color);
    }
...
struct VectorOf2 {
    double v[2];
    VectorOf2(double x = 0, double y = 0)
    {
        v[0] = x; v[1] = y;
    }
...
void DrawHermiteCurve1(HDC hdc, int x1, int y1, int x2, int y2, COLORREF c, int cx, int cy, int RR2)
{
    int a, b;
    double a0 = x1, a1 = 5,
        a2 = -3 * x1 - 2 * 5 + 3 * x2 - 5,
...
void DrawHermiteCurve2(HDC hdc, VectorOf2& p1, VectorOf2& T1, VectorOf2& p2, VectorOf2& T2, COLORREF c)
{
...
void Square(HDC hdc, int x, int y, int sideLength)
{
...
void DrawCircleModifiedBresenham(HDC hdc, int xc, int yc, int R, COLORREF color)
{
    int x = 0, y = R;
    int d = 1 - R;
    int c1 = 3, c2 = 5 - 2 * R;
...
//hermite square//
void FillWithHermiteCurves(HDC hdc, int xs, int ys, int xe, int ye, COLORREF color, int cx, int cy, int RR) {
    int dx = xe - xs;
    int dy = ye - ys;
    int yy = ye + RR;
    //SetPixel(hdc,xs,ys,color);
    DrawHermiteCurve1(hdc, xs, ys, xs, yy, color, cx, cy, RR);
    if (abs(dx) >= abs(dy))
    {
        int x = xs, xinc = dx > 0 ? 1 : -1;
        double y = ys, yinc = (double)dy / dx * xinc;
        while (x != xe)
        {
            ...
            SetPixel(hdc, x, Round(y), color);
            yy = Round(y) + RR;
            DrawHermiteCurve1(hdc, x, Round(y), x, yy, color, cx, cy, RR);
        }
    }
    else
    {
...
void DrawSquareFilling(HDC hdc, int xc, int yc, int dist, COLORREF c)
{
    int x1, y1;
    x1 = xc + dist;
    y1 = yc + dist;
    DrawLineParametric(hdc, xc, yc, x1, yc, c);
    DrawLineParametric(hdc, xc, yc, xc, y1, c);
    DrawLineParametric(hdc, xc, y1, x1, y1, c);
    DrawLineParametric(hdc, x1, yc, x1, y1, c);
    FillWithHermiteCurves(hdc, xc, yc, x1, yc, c, xc, yc, dist);
}
void FillWithBezierCurves(HDC hdc, int xs, int ys, int xe, int ye, COLORREF color, int cx, int cy, int RR, int RR1) {
    int dx = xe - xs;
    int dy = ye - ys;
    int xa = xs + RR;
...
void DrawRectangleFilling(HDC hdc, int xc, int yc, int dist, int dist1, COLORREF c)
{
    int x1, y1;
    x1 = xc + dist;
    y1 = yc + dist1;
    ...
    DrawLineParametric(hdc, xc, y1, x1, y1, c);
    DrawLineParametric(hdc, x1, yc, x1, y1, c);
    FillWithBezierCurves(hdc, xc, yc, xc, y1, c, xc, yc, dist, dist1);
}
//floodfill recursive//
void RecursiveFloodFill(HDC hdc, int x, int y, COLORREF borderColor, COLORREF fillColor)
{
    COLORREF colorOfPoint = GetPixel(hdc, x, y);
    if (colorOfPoint == borderColor || colorOfPoint == fillColor)
        return;
    SetPixel(hdc, x, y, fillColor);
    RecursiveFloodFill(hdc, x + 1, y, borderColor, fillColor);
    RecursiveFloodFill(hdc, x - 1, y, borderColor, fillColor);
    RecursiveFloodFill(hdc, x, y + 1, borderColor, fillColor);
    RecursiveFloodFill(hdc, x, y - 1, borderColor, fillColor);
}
...
void table2screen(HDC hdc, EdgeTable table, COLORREF c) {
    ...
    while (active.size() != 0) {
        ...
        for (iterator = active.begin(); iterator != active.end(); iterator++) {
            ...
            DrawLineParametric(hdc, ceil(firstNode.x), y, floor(secondNode.x), y, c);
        }
        y++;
...
void fillPolygonNonConvix(HDC hdc, POINT point[], int n, COLORREF c)
{
    EdgeTable table;
...
void TableToScreen(HDC hdc, edgeTable table, COLORREF color)
{
    ...
    for (int i = 0; i < 800; i++)
    {
        ...
        {
            DrawLineSimpleDDA(hdc, table[i].xLeft, i, table[i].xRight, i, color);
        }
    }
}
...
////////////////////////////// Clipping Functions ////////////////////////////
// used to manage mouse clicks
int clippingCounter = 0;
int rad, x_c, y_c;
struct point
{
    int x;
...
// POLYGON clipping using RECTANGLE //
...
// line clipping using RECTANGLE or SQUARE //
...
// point clipping using RECTANGLE or SQUARE //
...
////////////////////////// Spline ////////////////////////
void DrawCardinalSpline(HDC hdc, VectorOf2 P[], int n, double c, COLORREF C)
{
    double c1 = 1 - c;
    VectorOf2 T0 = c1 * ((P[2][0] - P[0][0]), (P[2][1] - P[0][1]));
    for (int i = 0; i < n - 1; i++)
    {
        VectorOf2 T1 = c1 * ((P[i + 1][0] - P[i - 1][0]), (P[i + 1][1] - P[i - 1][1]));
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        if ((number_of_rule >= 6 && number_of_rule <= 16) || (number_of_rule >= 21 && number_of_rule <= 26))
        {
            x = LOWORD(lParam);
            y = HIWORD(lParam);
        }
    ...
    case WM_RBUTTONDOWN:
        ...
        if (number_of_rule == 11)
        {
            x2 = LOWORD(lParam);
            y2 = HIWORD(lParam);
            DrawLineSimpleDDA(hdc, x, y, x2, y2, color);
        }
        ...
        if (number_of_rule == 6) {
            ...
            DrawCircleDirect(hdc, x, y, R, color);
        }
        if (number_of_rule == 7)//circleiter
        {
            ...
            y2 = (y2 - y) * (y2 - y);
            R = sqrt(x2 + y2);
            DrawCirclePolar(hdc, x, y, R, color);
        ...
        if (number_of_rule == 8)//circleitrativepolar
        {
            ...
            y2 = (y2 - y) * (y2 - y);
            R = sqrt(x2 + y2);
            DrawCircleIterativePolar(hdc, x, y, R, color);
        ...
        if (number_of_rule == 9)//circleBresenham
        {
            ...
            y2 = (y2 - y) * (y2 - y);
            R = sqrt(x2 + y2);
            DrawCircleBresenham(hdc, x, y, R, color);
        }
        if (number_of_rule == 10)// circleFasterVresenhamfaster
        {
            ...
            y2 = (y2 - y) * (y2 - y);
            R = sqrt(x2 + y2);
            DrawCircleModifiedBresenham(hdc, x, y, R, color);
        }
        ...
        if (number_of_rule == 23)
        {
            ...
            y2 = HIWORD(lParam) - y;
            R = sqrt(x2 * x2 + y2 * y2);
            DrawSquareFilling(hdc, x, y, R, color);
        }
        if (number_of_rule == 24)
        {
            x2 = LOWORD(lParam) - x;
            y2 = HIWORD(lParam) - y;
            DrawRectangleFilling(hdc, x, y, x2, y2, color);
        }
        ...
        else if (number_of_rule == 27 || number_of_rule == 28)
        {
            ...
            else if (Num_of_Points == 5)
            {
                ...
                if (number_of_rule == 27)
                    fillPolygonNonConvix(hdc, P, 5, color);
                else
        ...
        else if (number_of_rule == 29) //rectangle window
        {
            ...
            else if (clippingCounter == 1)
            {
                ...
                P[clippingCounter - 2].x = LOWORD(lParam);
                P[clippingCounter - 2].y = HIWORD(lParam);
                if (clippingCounter == 6)
                {
                    PolygonClip(hdc, P, 5, X_left, Y_top, X_right, Y_bottom, color);
                    clippingCounter = 2;
                }
                else clippingCounter++;
            }
        }
        ...
        else if (number_of_rule == 32)
        {
            ...
            else if (clippingCounter == 1)
            {
                ...
                x2 = LOWORD(lParam);
                y2 = HIWORD(lParam);
                R = sqrt((x2 - X_left) * (x2 - X_left) + (y2 - Y_top) * (y2 - Y_top));
                X_right = X_left + R;
                Y_bottom = Y_top + R;
                add_point(X_right, Y_bottom);
                Square(hdc, X_left, Y_top, R);
                add_shape(clipping, color);
                clippingCounter++;
        ...
        else if (number_of_rule == 34)
        {
            ...
            else if (clippingCounter == 1)
            {
                ...
                DrawCircleIterativePolar(hdc, x_c, y_c, rad, color);
                add_shape(clipping, color);
                clippingCounter++;
            }
            else if (clippingCounter == 2)
        ...
        {
            Pt[index] = VectorOf2(LOWORD(lParam), HIWORD(lParam));
            if (index == 3) {
